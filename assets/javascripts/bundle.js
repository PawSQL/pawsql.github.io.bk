...
(() => {
  ...
  function renderSearchDocument(document2, flag) {
    ...
    }, truncate(document2.text, 320)), document2.tags && document2.tags.map((tag) => /* @__PURE__ */ h("span", {
      class: "md-tag"
    ...
    const children = [
      ...
      ...more.length ? [
        /* @__PURE__ */ h("details", {
          ...
          tabIndex: -1
        }, more.length > 0 && more.length === 1 ? translation("search.result.more.one") : translation("search.result.more.other", more.length)), ...more.map((section) => renderSearchDocument(section, 1 /* TEASER */)))
      ] : []
    ];
  ...
  function mountAnnotation(el, container) {
    return defer(() => {
      const push$ = new Subject();
      push$.subscribe({
        next({ offset }) {
      ...
      const index2 = getElement(":scope > :last-child", el);
      const blur$ = fromEvent(index2, "mousedown", { once: true });
      push$.pipe(
  ...
  function mountAnnotationList(el, container, { print$: print$2 }) {
    ...
    return defer(() => {
      ...
      return merge(
        ...[...annotations].map(([, annotation]) => mountAnnotation(annotation, container))
      ).pipe(
  ...
  function fetchScripts() {
    return typeof mermaid === "undefined" || mermaid instanceof Element ? watchScript("https://unpkg.com/mermaid@9.0.1/dist/mermaid.min.js") : of(void 0);
  }
  function mountMermaid(el) {
    ...
    mermaid$.subscribe(() => {
      ...
      mermaid.mermaidAPI.render(id, el.textContent, (svg) => {
  ...
  function mountContentTabs(el) {
    ...
    return defer(() => {
      ...
      if (feature("content.tabs.link"))
        push$.pipe(skip(1)).subscribe(({ active }) => {
  ...
  function mountContent(el, { target$: target$2, print$: print$2 }) {
    return merge(
      ...getElements("pre:not(.mermaid) > code", el).map((child) => mountCodeBlock(child, { print$: print$2 })),
  ...
  var content$ = defer(() => merge(
    ...
    ...getComponentElements("content").map((el) => mountContent(el, { target$, print$ })),
